...
impl<P: PersistenceProvider + 'static> SubscriptionHandler<P> {
    ...
    pub async fn subscribe(&self, host: Host, key: String, user_id: UserId, channel_id: ChannelId) {
        ...
        self.watcher
            .watch(host.url, host.auth_header, key, callback);
    ...
    pub async fn restore_subscriptions(&self) -> Result<(), CoreError> {
        ...
        for subscription in subscriptions {
            ...
            let stream_url = format!("{}/{}", host.url, key);
            let callback =
                self.build_callback(subscription_id, channel_id, stream_url, key.clone());

            self.watcher
                .watch(host.url, host.auth_header, key.clone(), callback);
    ...
    fn build_callback(
        ...
        stream_url: String,
        key: String,
    ) -> impl Fn(StreamStatus) -> Pin<Box<dyn Future<Output = ()> + Send + 'static>>
    + Send
    + Sync
    + 'static {
        ...
        move |status: StreamStatus| {
            ...
            Box::pin(async move {
                ...
                match notifier
                    .notify(status, state, channel_id, stream_url, key)
                    .await
                    .unwrap()
                {
                    Some(StateChange::Online { message_id }) => persistence
                        .mark_subscription_online(subscription_id, message_id)
                        .await
                        .unwrap(),
                    Some(StateChange::Offline) => persistence
                        .mark_subscription_offline(subscription_id)
                        .await
                        .unwrap(),
                    None => {}
                }
